...
const cases = [
  ...
  ["confirmed malicious reputation", decide([finding("REPUTATION_CONFIRMED_MALICIOUS", "THREAT_INTELLIGENCE")], 90), "HIGH_RISK", true],
...
assert(cases.filter((entry) => entry[2] === "SUSPICIOUS").every((entry) => entry[1].evidenceGroups.filter((group) => group !== "SENSITIVE_INPUT").length >= 2), "SUSPICIOUS requires two behavioral groups");
