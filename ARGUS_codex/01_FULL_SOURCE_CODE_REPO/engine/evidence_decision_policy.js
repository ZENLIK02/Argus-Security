(function exposeArgusEvidencePolicy(root, factory) {
  ...
  const POLICY_VERSION = "evidence-first-v4";
  ...
  const GROUPS = Object.freeze({
    ...
    DOMAIN_RISK: new Set(["DOMAIN_BRAND_LOOKALIKE", "DOMAIN_PUNYCODE", "URL_OBFUSCATION", "URL_EXCESSIVE_SUBDOMAINS", "DOMAIN_HIGH_DIGIT_RATIO", "DECEPTIVE_BRAND_SUBDOMAIN", "BRAND_HOMOGRAPH_OR_TYPOSQUAT"]),
    GAMBLING_OPERATOR: new Set(["GAMBLING_CONTENT"]),
    SENSITIVE_ACTION: new Set(["SENSITIVE_ACTION_SURFACE"]),
    EXTERNAL_REPUTATION: new Set(["REPUTATION_RISKY_CONTEXT", "REPUTATION_RISKY_CATEGORY", "REPUTATION_CONFIRMED_MALICIOUS"])
  });
  ...
  function decide(input) {
    ...
    const direct = uniqueDirectEvidence(engineDirect.concat(telemetryDirect));
    ...
    if (!phaseFinal) {
      ...
      status = "HIGH_RISK";
      score = Math.max(70, Math.min(100, Number(legacy.score) || 70));
      confidence = "HIGH";
    ...
    } else if (identitySuspicious || (evidenceGroups.length >= 2 && observedGroups.length >= 1)) {
      status = "SUSPICIOUS";
    ...
    const warningAllowed = overlayAllowed || (phaseFinal && status === "RISKY_CONTEXT") || (phaseFinal && status === "SUSPICIOUS" && (identitySuspicious || evidenceGroups.length >= 2 && observedGroups.length >= 1));
    ...
    return {
      ...
      identityEvidence: { domainMismatch: identityMismatch, visualMatch: Boolean(identity.visualMatch), deceptiveSubdomain: Boolean(identity.deceptiveSubdomain), homographOrTyposquat: Boolean(identity.homographOrTyposquat) },
