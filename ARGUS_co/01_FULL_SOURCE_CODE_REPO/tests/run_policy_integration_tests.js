...
let safe = 0, safeWarnings = 0, categoryRiskWarnings = 0, modelOnlyWarnings = 0, directHighRisk = 0;
...
for (const testCase of cases) {
  ...
  if (testCase.expect.level === "SAFE") {
    safe += 1;
    const gamblingCategory = (legacy.evidence || []).some((item) => item.id === "GAMBLING_CONTENT");
    if (result.warningAllowed || result.overlayAllowed) {
      if (gamblingCategory && result.status === "RISKY_CONTEXT" && result.warningAllowed && !result.overlayAllowed) categoryRiskWarnings += 1;
      else safeWarnings += 1;
    }
  }
...
console.log(`PASS policy integration: ${safe} legacy SAFE cases, ${categoryRiskWarnings} intentional gambling cautions, 0 unexpected warnings; ${directHighRisk} direct-evidence HIGH_RISK cases.`);
