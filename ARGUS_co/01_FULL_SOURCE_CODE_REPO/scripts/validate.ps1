...
try {
  $javascriptFiles = @(
    ...
    "tests/run_domain_similarity_tests.js",
    "tests/run_reputation_tests.js",
    "tests/run_reputation_e2e.js",
    "tests/run_correlation_race_tests.js",
    "tests/run_scan_freshness_tests.js",
    "tests/run_shared_config_tests.js",
    "tests/run_feedback_endpoint_tests.js",
    "tests/run_exfiltration_pipeline_regressions.js",
    "engine/domain_similarity.js",
    "engine/reputation.js",
    "engine/reputation_client.js",
  ...
  node tests/run_domain_similarity_tests.js
  if ($LASTEXITCODE -ne 0) { throw "Offline brand-lookalike checks failed." }

  node tests/run_reputation_tests.js
  if ($LASTEXITCODE -ne 0) { throw "Reputation unit checks failed." }

  node tests/run_correlation_race_tests.js
  if ($LASTEXITCODE -ne 0) { throw "Correlation race checks failed." }

  node tests/run_scan_freshness_tests.js
  if ($LASTEXITCODE -ne 0) { throw "Scan freshness checks failed." }

  node tests/run_shared_config_tests.js
  if ($LASTEXITCODE -ne 0) { throw "Shared config checks failed." }

  node tests/run_feedback_endpoint_tests.js
  if ($LASTEXITCODE -ne 0) { throw "Feedback endpoint checks failed." }

  node tests/run_exfiltration_pipeline_regressions.js
  if ($LASTEXITCODE -ne 0) { throw "Exfiltration pipeline regression checks failed." }
  ...
  # Route-level backend tests (FastAPI TestClient; the file carries its own runner,
  # so pytest is optional). Requires `pip install -r backend/requirements.txt`.
  Push-Location (Join-Path $project "backend")
  try {
    & $pythonPath test_reputation_route.py
    if ($LASTEXITCODE -ne 0) { throw "Backend reputation route tests failed." }
  } finally {
    Pop-Location
  }

  # Real-backend reputation E2E (spawns uvicorn on a scratch port).
  $env:ARGUS_E2E_PYTHON = $pythonPath
  node tests/run_reputation_e2e.js
  if ($LASTEXITCODE -ne 0) { throw "Reputation end-to-end suite failed." }
