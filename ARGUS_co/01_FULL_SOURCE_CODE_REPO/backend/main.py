...
class ReputationRequest(BaseModel):
    # Canonical field is `hostname`; `host` and `domain` are backward-compatible
    # aliases. Only a hostname (metadata) is ever accepted — never a URL, path,
    # query, or body.
    ...
    def normalized_hostname(self) -> str:
        raw = str(self.hostname or self.host or self.domain or "")
        try:
            return normalize_hostname(raw)
        except ValueError:
            # Keep the pre-merge lenient behavior for the response echo: an invalid
            # hostname simply resolves to UNKNOWN instead of erroring.
            return raw.strip().lower().rstrip(".")


REPUTATION_SEED = load_reputation_seed()


def _seed_match(host: str) -> dict[str, Any] | None:
    """Exact or parent-domain match against the reviewed seed entries."""
    if host in REPUTATION_SEED:
        return REPUTATION_SEED[host]
    labels = host.split(".")
    for index in range(1, len(labels) - 1):
        parent = ".".join(labels[index:])
        if parent in REPUTATION_SEED:
            return REPUTATION_SEED[parent]
    return None


def build_reputation_result(host: str) -> dict[str, Any]:
    """Unified graded reputation verdict.

    Resolution order: local blocklist -> Google Web Risk (only when
    ARGUS_WEBRISK_API_KEY is configured; offline failure is non-fatal) -> reviewed
    seed entries -> UNKNOWN. `listed` mirrors verdict == MALICIOUS for
    backward compatibility with the pre-merge binary schema.
    """
    checked_at = datetime.now(timezone.utc).isoformat()
    result: dict[str, Any] = {
        "ok": True,
        "hostname": host,
        "host": host,      # backward-compatible echo
    ...
    if matched is not None:
        result.update({
            ...
            "categories": ["PHISHING_OR_MALWARE"],
        })
        return result

    webrisk = query_google_web_risk(host)
    if webrisk.get("available") and webrisk.get("malicious"):
        result.update({
            "listed": True,
            "verdict": "MALICIOUS",
            "confidence": "HIGH",
            "sources": ["GOOGLE_WEB_RISK"],
            "source": "GOOGLE_WEB_RISK",
            "matchedDomain": host,
            "categories": [str(item) for item in webrisk.get("categories", [])][:5],
        })
        return result

    seed = _seed_match(host)
    if seed is not None:
        verdict = str(seed.get("verdict", "RISKY_CONTEXT")).upper()
        if verdict == "RISKY_CATEGORY":
            verdict = "RISKY_CONTEXT"
        result.update({
            "listed": verdict == "MALICIOUS",
            "verdict": verdict,
            "confidence": str(seed.get("confidence", "MEDIUM")).upper(),
            "sources": [str(seed.get("source", "REVIEWED_SEED"))],
            "source": str(seed.get("source", "REVIEWED_SEED")),
            "matchedDomain": str(seed.get("hostname", host)),
            "categories": [str(item) for item in seed.get("categories", [])][:5],
            "firstSeen": seed.get("firstSeen"),
            "lastSeen": seed.get("lastSeen"),
        })
    return result
...
def health() -> dict[str, Any]:
    return {
        ...
        "reputationSeedEntries": len(REPUTATION_SEED),
    }


def _reload() -> dict[str, Any]:
    global BLOCKLIST_DOMAINS, REPUTATION_SEED
    BLOCKLIST_DOMAINS = load_blocklist()
    REPUTATION_SEED = load_reputation_seed()
    return {
        "ok": True,
...
def reputation(request: ReputationRequest) -> dict[str, Any]:
    ...
    received — no URL, path, query, cookies, or page content. Graded verdicts:
    local blocklist -> MALICIOUS; Google Web Risk (optional, key required) ->
    MALICIOUS; reviewed seed -> RISKY_CONTEXT; otherwise UNKNOWN.
    """
    return build_reputation_result(request.normalized_hostname())
