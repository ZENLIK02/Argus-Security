...
ARGUS 5.1 routes every visible decision through `engine/evidence_decision_policy.js`. The legacy evidence engine and local model are producers, not warning authorities. A model-only score, including 100/100, can produce only `MONITORING` or `UNCERTAIN`; it cannot produce `SUSPICIOUS`, `HIGH_RISK`, or an overlay.
