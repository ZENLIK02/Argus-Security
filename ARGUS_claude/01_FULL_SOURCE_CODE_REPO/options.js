...
const elements = {
  ...
  useReputation: document.getElementById("useReputation"),
...
function renderSettings(settings) {
  ...
  elements.useReputation.checked = settings.useReputation;
...
async function saveSettings() {
  const settings = normalizeSettings({
    ...
    useReputation: elements.useReputation.checked,
...
function normalizeSettings(settings) {
  ...
  return {
    ...
    // the service worker in agreement (F16). The reputation endpoint is pinned the
    // same way — the lookup only ever talks to the local backend.
    ...
    reputationEndpoint: ArgusFeedbackEndpoint.normalizeFeedbackEndpoint(raw.reputationEndpoint, DEFAULT_SETTINGS.reputationEndpoint),
