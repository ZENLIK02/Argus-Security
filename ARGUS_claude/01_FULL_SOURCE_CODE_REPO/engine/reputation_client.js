(function exposeArgusReputationClient(root, factory) {
  ...
  async function getCached(domain, deps) {
    try {
      ...
      return Reputation.isFresh(entry, nowOf(deps), Reputation.REPUTATION_TTL_MS) ? entry : null;
    } catch (error) {
      return null;
    }
  }
  ...
  async function resolve(signals, settings, tabId, deps) {
    const diag = {
      enabled: Boolean(settings.useReputation),
      endpoint: settings.reputationEndpoint,
      domain: signals.domain,
      queried: false, cache: "n/a", listed: false, source: "NONE", applied: false, reason: ""
    };
    if (!settings.useReputation || !Reputation) {
      diag.reason = settings.useReputation ? "module_unavailable" : "disabled";
    ...
    if (cached) {
      ...
      diag.applied = Boolean(cached.available && cached.listed);
  ...
  async function refresh(domain, tabId, settings, deps) {
    ...
    try {
      ...
      diag.verdict = verdict.listed ? "malicious" : "clean";
    } catch (error) {
