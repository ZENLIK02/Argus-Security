...
try {
  importScripts("engine/shared_lists.js", "engine/domain_similarity.js", "engine/feature_extractor.js", "engine/trained_model.js", "engine/argus_engine.js", "engine/evidence_decision_policy.js", "engine/navigation_session_guard.js", "engine/network_correlation.js", "engine/scan_freshness.js", "engine/feedback_endpoint.js", "engine/reputation.js", "engine/reputation_client.js");
...
async function ensureSettingsInitialized() {
  try {
    ...
    const missingReputation = !raw || typeof raw !== "object" ||
      raw.useReputation === undefined || !raw.reputationEndpoint;
    if (missingReputation) {
      const merged = normalizeSettings(raw);
      await chrome.storage.local.set({ [SETTINGS_KEY]: merged });
      console.log("[Project Argus] settings initialized; reputation enabled:", merged.useReputation, merged.reputationEndpoint);
    }
  } catch (error) {
...
async function handlePageScan(pageData, sender) {
  ...
  const finalRisk = {
    ...ruleRisk,
    ...policyDecision,
    category: policyDecision.status === "SAFE" ? "SAFE" : ["MONITORING", "UNCERTAIN"].includes(policyDecision.status) ? "UNCONFIRMED" : ruleRisk.category,
...
async function resolveReputationContext(signals, settings, tabId) {
  if (typeof ArgusReputationClient === "undefined") {
    return { context: undefined, diag: { enabled: Boolean(settings.useReputation), reason: "client_unavailable", cache: "n/a" } };
  }
...
function normalizeSettings(settings) {
  ...
  return {
    ...
    useReputation: raw.useReputation !== false,
...
function publicSettings(settings) {
  return {
    ...
    useReputation: settings.useReputation
  };
}
