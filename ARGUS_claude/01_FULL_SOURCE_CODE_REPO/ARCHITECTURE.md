...
The evidence policy distinguishes allowlisted direct evidence from correlated groups and weak context. `HIGH_RISK` requires direct evidence. `SUSPICIOUS` requires at least two independent groups. Model output is adapted as `overallRiskProbability`; behavior-specific probabilities are marked unavailable rather than fabricated.

Project Argus 4.2 is a local evidence ensemble. It does not call an external AI service. The Chrome extension is the authoritative detector; the FastAPI process is an optional static demo server.
