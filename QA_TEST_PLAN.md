...
## ARGUS 5.3 identity-context check

- Open `http://localhost:8000/test-site/fake-kbank-identity.html` with sensitivity set to Balanced.
- Expected after the final scan: `RISKY_CONTEXT`, context `BANKING_LENDING`, claimed identity `Kasikornbank`, actual domain `localhost`, badge-stage caution.
- Click the harmless “Secure account login” button. Expected after the interaction rescan: warning stage `INTERACTION` and the detail panel expands without a blocking overlay.
- Repeat in Conservative mode: no visible caution until the sensitive field is approached.
- Repeat in Protective mode: the early caution may appear from the banking context before interaction.
- Never enter real credentials; the fixture is local and its dummy endpoint discards the request body.
